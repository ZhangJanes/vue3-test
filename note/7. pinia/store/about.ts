...
//       this.loveList.push(message)
//       this.sum = this.loveList.length
//     },
//   },
//   state() {
//     return {
//       sum: 0,
//       loveList:
//         (JSON.parse(localStorage.getItem('loveList') as string) as string[]) || ([] as string[]),
//     }
//   },
//   getters: {
//     // 可以理解为计算属性
...
export const useAboutStore = defineStore('about', () => {
  ...
  const loveList = ref<string[]>(
    JSON.parse(localStorage.getItem('loveList') || '[]'), // 初始化处理
  )
  ...
  const getTalkList = async () => {
    try {
      ...
      loveList.value.push(data.content)
      sum.value = loveList.value.length

      // 持久化到 localStorage
      localStorage.setItem('loveList', JSON.stringify(loveList.value))
    } catch (error) {
